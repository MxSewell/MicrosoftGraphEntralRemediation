...
[CmdletBinding()]
param(
    [string]$CsvPath    = (Join-Path $PSScriptRoot 'BulkMFAUsers.csv'),
    [string]$LogPath    = (Join-Path $PSScriptRoot 'PhoneMethodResults.csv'),
    [ValidateSet('enabled','disabled','leave')]
    [string]$SmsSignIn  = 'leave',
    [switch]$WhatIf

    <# Entra Group
    [string]$EntraGroupID   = '61b6e03a-9c0a-4ad5-95b0-0776a4e3d034'#>
)
...
# ===Step 7: Loop the CSV + add logging===
$WhatIf = $false
...
foreach ($r in $rows) {
    ...
    try {
        ...
        # ----- Optional: Update SMS sign-in toggle
